...

{
  home.packages = with pkgs; [
    ...
    fish protonmail-bridge proton-pass starship nushell input-leap
  ];
  ...
  programs.nushell = {
    ...
    shellAliases = {
      zed = "zeditor";
      rebuild = "sudo nixos-rebuild switch --show-trace --flake git+https://github.com/hiarthurbr/nix";
      update = "nix-channel --update";
      upgrade = "nix-env --upgrade";
    ...
    configFile.text = ''
      ...
      def open-project [project: string] {
        ...
        if ($devenv_path | path exists) {
          ...
          run-external $editor_cmd ($project_path | path expand)
          nix-shell --show-trace -p ...$packages "--command" $"nu -e 'cd ($project_path); ($aliases | str join '; '); ($functions | str join '; ')'"
        } else {
